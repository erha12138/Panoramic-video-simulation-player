...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        self.obj = tf.compat.v1.reduce_sum(tf.compat.v1.multiply(
                       tf.compat.v1.log(tf.compat.v1.reduce_sum(tf.compat.v1.multiply(self.out, self.acts),
                                            reduction_indices=1, keep_dims=True)),
                       -self.act_grad_weights)) \
                   + ENTROPY_WEIGHT * tf.compat.v1.reduce_sum(tf.compat.v1.multiply(self.out,
                                                           tf.compat.v1.log(self.out + ENTROPY_EPS)))

        # Combine the gradients here
        self.actor_gradients = tf.compat.v1.gradients(self.obj, self.network_params)
...
class CriticNetwork(object):
    ...
    def __init__(self, sess, state_dim, learning_rate):
        ...
        self.td = tf.compat.v1.subtract(self.td_target, self.out)

        # Mean square error
        self.loss = tflearn.mean_square(self.td_target, self.out)

        # Compute critic gradient
        self.critic_gradients = tf.compat.v1.gradients(self.loss, self.network_params)
